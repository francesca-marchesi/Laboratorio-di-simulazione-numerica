...
{
	//double k_elastic = 1;
	return val*val*val*val-5./2.*val*val;
}
...
{
    double mu = 0.699043;
    double sigma = 0.547852;
    return exp(-0.5*(v*v-2*v*mu+mu*mu)/(sigma*sigma))+exp(-0.5*(v*v+2*v*mu+mu*mu)/(sigma*sigma));
    //return 1.0;
}
...
double variationalWaveFunction_second(double v)
{
    double mu = 0.699043;
    double sigma = 0.547852;
	return (v-mu)/(sigma*sigma)*exp(-0.5*(v*v-2*v*mu+mu*mu)/(sigma*sigma))+(v+mu)/(sigma*sigma)*exp(-0.5*(v*v+2*v*mu+mu*mu)/(sigma*sigma));
	//return 0.0;
}
