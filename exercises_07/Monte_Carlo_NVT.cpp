...
void Averages(int iblk) //Print results for current block
{
   ...
   //const int wd=12;
    ...
    Epot.open("epot.solid",ios::app);
    Pres.open("pres.solid",ios::app);
    Gofr.open("gofr.solid",ios::app);
    Gave.open("gave.solid",ios::app);
...
//g(r)
    for(int i=igofr; i<igofr+nbins; ++i){
        r=(i-igofr)*bin_size;   //estremo sinistro del bin
        deltaV=(4./3.)*M_PI*(pow(r+bin_size,3)-pow(r,3));
        gdir=blk_av[i]/(rho*npart*deltaV)/blk_norm;
        glob_av[i] += gdir;
        glob_av2[i] += gdir*gdir;
        err_gdir=Error(glob_av[i],glob_av2[i],iblk);
        
        Gofr << iblk << "," << r <<  "," << glob_av[i]/(double)iblk << "," << err_gdir <<endl;
    ...
    if(iblk==nblk){ 
        for(int i=igofr; i<igofr+nbins; ++i){
            r=(i-igofr)*bin_size;
            Gave << igofr << "," << r << "," <<glob_av[i]/(double)iblk << "," << err_gdir <<endl;
        }
    }
